...
r = d %>% filter(lim_free_mem == 1e+05 ) %>% group_by(fs, storage, processors, phase) %>% summarise(obj_per_s = mean(obj_per_s))
...
ggsave("tp-phase-unlimitedMemory.png")
...
ggsave("tp-phase-by-storage-unlimitedMemory.png")
...
for (dset in c(10, 50)){
	...
	ggsave(paste(dset,"tp-fs.png", sep=""))

	r = filter(d, phase == "b", lim_free_mem == 1000, processors == 2, data_sets == dset )

	ggplot(r, aes(iteration, obj_per_s, col=storage)) + geom_point() + facet_grid(fs ~ ., switch = 'y') +  scale_y_log10() + ylab("objects/s") # scales="free_y",
	ggsave(paste(dset,"tp-2proc-fs.png", sep=""))
